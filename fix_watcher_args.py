...
content = content.replace(
    ...
    "Ok(enqueue_hot_path_delta_count(watch_root, project_code, path, priority, Some(guard), ingress)? > 0)"
)
content = content.replace(
    "Ok(stage_hot_path_delta_count(store, watch_root, path, priority, Some(guard))? > 0)",
    "Ok(stage_hot_path_delta_count(store, watch_root, project_code, path, priority, Some(guard))? > 0)"
)
