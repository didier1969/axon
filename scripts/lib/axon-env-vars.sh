...
# REQ-AXO-241 — Single source of truth for AXON_* env var
...
#
# Default policy: any var matching AXON_* (and a small set of
...
# policy? AXON_* is the canonical Axon namespace; OMP_* is the narrow
# set of OpenMP-specific knobs we propagate (OMP_NUM_THREADS,
...
axon_env_var_in_prefix_allowlist() {
    case "$1" in
        AXON_*|OMP_NUM_THREADS|OMP_WAIT_POLICY|MALLOC_*) return 0 ;;
        *) return 1 ;;
    esac
}
