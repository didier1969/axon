...
IDs per DEC-AXO-085 (ID format); SQL renamed from cypher per MIL-AXO-017 (AGE retirement); `documente` / `save observation` → `document_intent` (REQ-AXO-141 (auto-classifier)), never `soll_manager`.
...
Async: `job_id` → `job_status`; `wait=true` blocks. `soll_work_plan` / `soll_verify_requirements` shape: CPT-AXO-024 (LLM-only doc methodology).
