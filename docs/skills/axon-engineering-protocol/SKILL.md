...
| `soll_manager` failure (REQ-AXO-147 slice + REQ-AXO-325 status slice) | inspect `data.parameter_repair.{invalid_field, category?, supplied_value?, accepted_values?, normalization_hint?, canonical_source?, supplied_source_id?, supplied_target_id?, follow_up_tools, hint}` + optional `data.example_valid_call`. Categories: `entity` (unknown type → accepted_values list), `project_code` (unregistered → `project_registry_lookup` / `axon_init_project`), `relation_type` (forbidden link → `soll_relation_schema`), `data.title|description` (writer error — REQ-AXO-091 placeholder bug), `target_id` (target not found → `sql`), `status` (hors-vocabulaire DEC-PRO-100 → accepted_values=[current,planned,delivered,superseded,rejected] + `normalization_hint` mapping legacy→canonical) |
...
| CPT → CPT | INHERITS_FROM (default), REFINES (REQ-AXO-274 phase 2 — cross-project specialization), SUPERSEDES (REQ-AXO-326 — replacement edge) |
| PIL → PIL | SUPERSEDES (REQ-AXO-326 — pillar absorbed/fused into another, e.g. PIL-AXO-006 → PIL-AXO-004) |
| GUI → GUI | INHERITS_FROM (default, cross-project propagation), SUPERSEDES (REQ-AXO-326 — replacement, e.g. GUI-AXO-1001 → GUI-PRO-100) |
| REQ → REQ | REFINES (default), BELONGS_TO, SUPERSEDES (REQ-AXO-326 — duplicate/replacement, e.g. REQ-AXO-207/208 → REQ-AXO-198) |
