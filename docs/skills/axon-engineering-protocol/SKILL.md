...
- `soll_export` is **disabled by default in production** until a retention or replacement policy lands. To opt in, set `AXON_SOLL_EXPORT_ENABLED=1` (or `true`/`yes`/`on`); without it the tool returns a success-shaped contract with `data.disabled=true`, `data.reason`, and `data.enable_via`, and writes nothing to disk. `axon_commit_work` keeps working because the disabled branch returns the same response shape (just no file). Tests keep the legacy enabled-by-default behaviour via `cfg(test)`. The 764 accumulated `docs/vision/SOLL_EXPORT_*.md` files were deleted on 2026-05-01 (REQ-AXO-126).
