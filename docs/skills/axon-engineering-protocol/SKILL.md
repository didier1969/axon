...
IDs per DEC-AXO-085 (ID format); SQL renamed from cypher per MIL-AXO-017 (AGE retirement); graph traversal tools (`impact`, `path`, `why`, `anomalies`, `retrieve_context`) are thin wrappers on `db/ddl/04_graph_functions.sql` (`public.impact`/`callers_of`/`why_chain`/`blast_radius`/`path`/`retrieve_context_v2`) over `public.Edge` (REQ-AXO-296 + REQ-AXO-297 dual-write + REQ-AXO-299 tool bascule); `documente` / `save observation` → `document_intent` (REQ-AXO-141 (auto-classifier)), never `soll_manager`.
