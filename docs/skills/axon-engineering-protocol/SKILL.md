...
- `soll_query_context`, `soll_work_plan`, and `anomalies` with an unregistered `project_code` return `data.status="wrong_project_scope"`, `data.registered_project_codes` (array of valid codes), and `data.operator_guidance.follow_up_tools=["project_registry_lookup", "axon_init_project"]`. Previously soll_query_context returned the framework's generic "Invalid arguments"; soll_work_plan and anomalies silently returned `Status: ok` with empty/all-zero Evidence (REQ-AXO-043).
