...
| Task | Tool |
|---|---|
| Find symbol (multi-token, underscore-aware REQ-AXO-088) | `query` — under `AXON_DB_BACKEND=postgres` (MIL-AXO-015 P6), Symbol semantic search and the chunk-fallback search emit schema-qualified queries when a single project is supplied; pgvector `<=>` cosine distance powers Symbol semantic ANN; `project="*"` cross-schema search lands in P9 |
