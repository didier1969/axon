...
- watcher telemetry still does not expose the policy `rule_id` for rejected directory events
...
The policy is no longer only locally qualified.

Fresh validation executed on this branch:
...
Result at qualification time:
- targeted indexing policy tests passed
- targeted scanner integration tests passed
- the full `axon-core` crate test suite passed, so this policy has now been exercised as part of the crate-wide release gate
