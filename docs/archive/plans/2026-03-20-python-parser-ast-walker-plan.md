# Python Parser AST Walker Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite the Python parser in Rust using a recursive AST walker to extract functions, classes, function calls, and module imports, enabling full semantic intelligence.

**Architecture:** We will abandon `QueryCursor` in `python.rs` and implement a `walk` function that traverses the Tree-sitter AST. It will maintain `class_name` and `function_name` context to accurately attribute `CALLS` relations.

**Tech Stack:** Rust, Tree-sitter (Python WASM).

---

### Task 1: Scaffolding and Basic AST Walk

**Files:**
- Modify: `src/axon-core/src/parser/python.rs`

**Step 1: Rewrite the base structure**
Replace the current `parse` method and add the helper methods.
...
**Step 2: Commit**
```bash
git add src/axon-core/src/parser/python.rs
git commit -m "refactor(parser): setup recursive AST walker for python"
```

---

### Task 2: Implement Class and Function Extraction

**Files:**
- Modify: `src/axon-core/src/parser/python.rs`

**Step 1: Implement extract_class and extract_function**
Add these methods to `PythonParser`:
...
```bash
git add src/axon-core/src/parser/python.rs
git commit -m "feat(parser): implement class and function extraction for python"
```

---

### Task 3: Implement Call and Import Extraction

**Files:**
- Modify: `src/axon-core/src/parser/python.rs`

**Step 1: Implement extract_call and extract_import**
Add these methods to `PythonParser`:
...
```bash
git add src/axon-core/src/parser/python.rs
git commit -m "feat(parser): extract calls and imports for python taint analysis"
```
...
**Step 1: Update tests**
Replace the existing test module with this comprehensive test:
...
```bash
git add src/axon-core/src/parser/python.rs
git commit -m "test(parser): verify new python AST walker extracts calls and imports"
```