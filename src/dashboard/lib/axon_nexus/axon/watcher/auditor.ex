defmodule Axon.Watcher.Auditor do
  ...
  # Wait 3s after the LAST file to audit
  @idle_timeout 3_000
  # Force audit at least every 30s during continuous ingestion
  @max_delay 30_000
  ...
  def handle_info({:bridge_event, {:file_indexed, _path, _status}}, state) do
    ...
    new_max =
      state.max_delay_timer || Process.send_after(self(), :trigger_heavy_audit, @max_delay)
  ...
  def handle_info(
        {:bridge_event, %{"id" => id, "result" => %{"content" => [%{"text" => text} | _]}}},
        state
      )
    ...
    if project_name do
      ...
      if project =
           Axon.Watcher.Repo.get_by(Axon.Watcher.Tracking.IndexedProject, name: project_name) do
        # Coverage 0 for now
