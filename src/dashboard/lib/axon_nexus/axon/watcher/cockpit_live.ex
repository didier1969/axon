  ...
  def render(assigns) do
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <div class="stat"><label>TOTAL_INGESTED</label> <span style="color: var(--neon-blue);"><%= @live.total_ingested %></span></div>
      ...
      <div class="card">
        <div class="card-title">
          <svg style="width:18px;height:18px" viewBox="0 0 24 24"><path fill="currentColor" d="M21,16.5C21,16.88 20.79,17.21 20.47,17.38L12.57,21.82C12.41,21.94 12.21,22 12,22C11.79,22 11.59,21.94 11.43,21.82L3.53,17.38C3.21,17.21 3,16.88 3,16.5V7.5C3,7.12 3.21,6.79 3.53,6.62L11.43,2.18C11.59,2.06 11.79,2 12,2C12.21,2 12.41,2.06 12.57,2.18L20.47,6.62C20.79,6.79 21,7.12 21,7.5V16.5Z" /></svg>
          UNIT 02: PARSER MATRIX (POD B)
        </div>
        ...
        <div style="margin-top: 15px;">
          ...
          <% else %>
            <div style="font-size: 0.7rem; color: var(--text-dim); font-family: monospace;">IDLE_WAITING_FOR_TASKS</div>
          <% end %>
        </div>
      </div>
      ...
      <div class="card" style="border-color: var(--neon-blue);">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
          ...
          <button phx-click="toggle_monitoring" class="btn"><%= if @monitoring_active, do: "Pause", else: "Resume" %></button>
      ...
      <div class="card" style="grid-column: span 3; background: #000; border-color: #222;">
        <div class="card-title">
          <svg style="width:18px;height:18px" viewBox="0 0 24 24"><path fill="currentColor" d="M13,9H11V7H13M13,17H11V11H13M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z" /></svg>
        ...
        <div style="font-family: monospace; font-size: 0.75rem; height: 150px; overflow-y: hidden; display: flex; flex-direction: column-reverse;">
          <%= for file <- @live.last_files do %>
            <div style="padding: 4px 0; border-bottom: 1px solid #111; display: flex; gap: 15px;">
              <span style="color: var(--text-dim);"><%= String.slice(DateTime.to_iso8601(file.time), 11, 8) %></span>
          ...
          <%= if Enum.empty?(@live.last_files) do %>
            <div style="color: var(--text-dim); text-align: center; margin-top: 60px;">SYSTEM_IDLE: AWAITING_INGESTION_DATA</div>
          <% end %>
        </div>
      </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 15px;">
          ...
          <% else %>
            <%= for {dir, d_stats} <- @live.directories do %>
              <% percent = if d_stats.total > 0, do: trunc((d_stats.completed / d_stats.total) * 100), else: 0 %>
              <div style="border-left: 2px solid var(--neon-blue); padding-left: 12px; background: rgba(0,242,255,0.02); padding-bottom: 8px;">
                <div style="display: flex; justify-content: space-between; align-items: baseline;">
                  <div style="font-size: 0.85rem; font-weight: 700; color: #fff; text-transform: uppercase;"><%= dir %></div>
                  <div style="font-size: 0.65rem; color: var(--text-dim);">
                    <%= if d_stats.last_update, do: String.slice(DateTime.to_iso8601(d_stats.last_update), 11, 8), else: "PENDING" %>
                  </div>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 8px; font-size: 0.7rem; font-family: monospace;">
                  ...
                  <span style="color: var(--neon-red);"><%= if d_stats.failed > 0, do: "#{d_stats.failed} FAIL", else: "" %></span>
