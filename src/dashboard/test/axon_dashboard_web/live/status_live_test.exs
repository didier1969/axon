...
defmodule AxonDashboardWeb.StatusLiveTest do
  ...
  test "renders operator cockpit sections without external cdn assets", %{conn: conn} do
    ...
    assert html =~ "Memory"
    assert html =~ "Ingress"
    assert html =~ "Couverture fichiers"
    assert html =~ "Structure graphe"
    assert html =~ "Enrichissement sémantique"
    assert html =~ "Alignement SOLL"
    assert html =~ "Files Completed"
    assert html =~ "Files Graph Ready"
    assert html =~ "Files With Semantic Coverage (Derived)"
