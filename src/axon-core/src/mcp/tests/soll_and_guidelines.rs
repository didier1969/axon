...
// REQ-AXO-91577 — PRO sentinel project_code self-heals when the registry
// entry was lost (live regression). Sentinel project codes represent
// cross-tenant logical scopes (e.g. PRO = cross-project methodology surface,
// cf. PIL-AXO-9003 Two-Sided Identity) and must remain usable for
// `soll_manager` mutations even if the registry row is missing.
#[test]
fn test_pro_sentinel_self_heals_when_registry_entry_missing() {
    let server = create_test_server();

    // Simulate the live regression : registry entry deleted post-bootstrap.
    server
        .graph_store
        .execute("DELETE FROM soll.ProjectCodeRegistry WHERE project_code = 'PRO'")
        .unwrap();
    let pre = server
        .graph_store
        .query_count(
            "SELECT count(*) FROM soll.ProjectCodeRegistry WHERE project_code = 'PRO'",
        )
        .unwrap();
    assert_eq!(pre, 0, "PRO registry row must be deleted before test");
    ...
    let req = serde_json::json!({
        ...
        "params": {
            ...
            "arguments": {
                ...
                "data": {
                    "project_code": "PRO",
                    "title": "Sentinel self-heal smoke",
                    "description": "Body content sufficient to pass soll_validate criteria.",
    ...
    assert!(
        !is_error,
        "PRO sentinel creation should succeed via self-heal, got: {content}"
    );
    ...
    assert_eq!(
        post, 1,
        "PRO registry row must be auto-re-registered after sentinel use"
    );
}

// REQ-AXO-91577 — Non-sentinel + unregistered project_code must still be
// rejected. Sentinel allowlist must not whitelist all unknown codes.
#[test]
fn test_non_sentinel_unregistered_code_still_rejected() {
    let server = create_test_server();
        ...
        "params": {
            ...
            "arguments": {
                ...
                "data": {
                    "project_code": "ZZZ",
                    "title": "Should reject",
                    "description": "Non-sentinel + unregistered must be rejected.",
                    "attach_to": "GUI-PRO-001",
                    "relation_type": "INHERITS_FROM",
                    "status": "current"
                }
    ...
    assert!(
        is_error,
        "Non-sentinel unregistered code should be rejected, got: {content}"
    );
}
