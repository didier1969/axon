...
fn test_superseded_update_without_replacement_or_rationale_is_rejected() {
    ...
    assert_eq!(
        res["isError"], true,
        "Must be rejected when superseded_by and rationale are missing"
    );
...
fn test_rejected_update_without_rationale_is_rejected() {
    ...
    assert_eq!(
        res["isError"], true,
        "Must be rejected when retirement reason is missing"
    );
    let text = res["content"][0]["text"].as_str().unwrap_or_default();
    assert!(
...
fn test_superseded_update_with_superseded_by_establishes_edge_and_succeeds() {
    ...
    let raw = server
        ...
        .unwrap();
    assert!(
        raw.contains("superseded"),
        "Expected status superseded, got: {raw}"
    );
    ...
    assert_eq!(
        edge_count, 1,
        "SUPERSEDES edge must be created from replacement to retired node"
    );
}
...
fn test_superseded_update_with_existing_incoming_edge_succeeds() {
    ...
    assert!(
        res["isError"].is_null() || res["isError"] == false,
        "Must succeed with existing incoming SUPERSEDES edge: {res:?}"
    );
}
...
fn test_req_902579_supersedes_multi_sources_fanout_and_same_source_rejection() {
    ...
    let link_res1 = server
        ...
        .expect("must answer");
    assert!(
        link_res1["isError"].is_null() || link_res1["isError"] == false,
        "First SUPERSEDES must succeed: {link_res1:?}"
    );
    ...
    assert_eq!(
        incoming_count, 2,
        "Both SUPERSEDES edges must exist in soll.Edge"
    );
    ...
    let link_dup = server
        ...
        .expect("must answer");
    assert_eq!(
        link_dup["isError"], true,
        "Critère 2: Same source PIL-FAN-002 cannot supersede PIL-FAN-001 twice"
    );
    ...
    assert!(
        children_res["data"]["superseded_by_all"].is_array(),
        "superseded_by_all must be an array"
    );
    let all_reps = children_res["data"]["superseded_by_all"]
        .as_array()
        .unwrap();
    assert_eq!(all_reps.len(), 2);
    let children_text = children_res["content"][0]["text"]
        .as_str()
        .unwrap_or_default();
    assert!(
        children_text.contains("PIL-FAN-002") && children_text.contains("PIL-FAN-003"),
        "soll_children must mention both replacements: {children_text}"
    );
    ...
    assert!(
        why_text.contains("PIL-FAN-002") && why_text.contains("PIL-FAN-003"),
        "why must mention both replacements: {why_text}"
    );
}
