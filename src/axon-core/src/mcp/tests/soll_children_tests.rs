...
fn test_targets_semantic_traversal_in_both_directions() {
    ...
    assert_eq!(extract_node_ids(&mil_children_rel), vec!["REQ-OPV-994".to_string()]);
    ...
    let mil_parents_text = mil_parents["content"][0]["text"].as_str().unwrap_or_default();
    assert!(
        mil_parents_text.contains("direction=\"children\"") || mil_parents_text.contains("direction=\\\"children\\\""),
    ...
    assert_eq!(extract_node_ids(&req_parents), vec!["MIL-OPV-084".to_string()]);

    // 5. Semantic parents on REQ with relation_type="TARGETS"
    let req_parents_rel = server
        .axon_soll_children(&json!({ "id": "REQ-OPV-994", "direction": "parents", "relation_type": "TARGETS" }))
        .expect("must answer");
    assert_eq!(req_parents_rel["data"]["count"], 1);
    assert_eq!(extract_node_ids(&req_parents_rel), vec!["MIL-OPV-084".to_string()]);
    ...
    let req_children_text = req_children["content"][0]["text"].as_str().unwrap_or_default();
    assert!(
        req_children_text.contains("direction=\"parents\"") || req_children_text.contains("direction=\\\"parents\\\""),
...
fn test_solves_semantic_traversal() {
    ...
    let dec_children = server
        ...
        .expect("must answer");
    assert_eq!(extract_node_ids(&dec_children), vec!["REQ-SLV-001".to_string()]);

    // REQ parents -> DEC
    let req_parents = server
        .axon_soll_children(&json!({ "id": "REQ-SLV-001", "direction": "parents" }))
        .expect("must answer");
    assert_eq!(extract_node_ids(&req_parents), vec!["DEC-SLV-001".to_string()]);
}
...
fn test_mixed_graph_strictly_excludes_non_hierarchical_relations_from_filiation() {
    ...
    assert!(!got_ids.contains(&"REQ-MIX-103".to_string()), "BLOCKED_BY is a dependency, not a child");
    assert!(!got_ids.contains(&"DEC-MIX-001".to_string()), "BLOCKED_BY is a dependency, not a child");
    assert!(!got_ids.contains(&"REQ-MIX-104".to_string()), "SUPERSEDES is a retirement, not a child");
    ...
    let non_hier = server
        .axon_soll_children(&json!({ "id": "REQ-MIX-100", "direction": "children", "relation_type": "BLOCKED_BY" }))
        .expect("must answer");
...
fn test_node_payload_carries_physical_source_and_target_ids() {
    ...
    assert_eq!(n["source_id"], "MIL-PLD-001", "physical source_id must match Edge.source_id");
    assert_eq!(n["target_id"], "REQ-PLD-001", "physical target_id must match Edge.target_id");
}
...
fn test_opposite_direction_check_respects_relation_filter() {
    ...
    let res_targets = server
        .axon_soll_children(&json!({ "id": "MIL-FLT-001", "direction": "parents", "relation_type": "TARGETS" }))
        .expect("must answer");
    assert_eq!(res_targets["data"]["count"], 0);
    let text_targets = res_targets["content"][0]["text"].as_str().unwrap_or_default();
    assert!(
    ...
    let res_belongs = server
        .axon_soll_children(&json!({ "id": "MIL-FLT-001", "direction": "parents", "relation_type": "BELONGS_TO" }))
        .expect("must answer");
    assert_eq!(res_belongs["data"]["count"], 0);
    let text_belongs = res_belongs["content"][0]["text"].as_str().unwrap_or_default();
    assert!(
...
fn test_capping_at_200_items_sets_capped_flag() {
    ...
    assert_eq!(res["data"]["capped"], true, "capped flag must be true when reaching 200");
    let text = res["content"][0]["text"].as_str().unwrap_or_default();
    assert!(text.contains("capped at 200"), "text must announce capping: {text}");
}
