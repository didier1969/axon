...
fn embedder_state() -> SubsystemState {
    runtime_readiness::snapshot_subsystem_reports()
        .into_iter()
        .find(|report| report.subsystem == "embedder")
        .unwrap()
        .state
}
...
fn terminal_inference_failure_invalidates_ready_and_recovers() {
    if !enter_isolated_case("terminal_inference_failure_invalidates_ready_and_recovers") {
        return;
    }
    ...
    let mut worker = None;
    {
        ...
        assert!(
            error
                .to_string()
                .contains("synthetic inference allocation failure"),
            "{error:#}"
        );
        assert!(worker.is_none(), "both failed workers must be shut down");
        assert!(
            matches!(embedder_state(), SubsystemState::Failed { ref reason }
            if reason == "isolated_query_worker_inference_failed"),
            "successful handshakes must not mask terminal inference failure: {:?}",
            embedder_state()
        );
    }
...
fn successful_inference_refreshes_readiness_without_restarting_worker() {
    if !enter_isolated_case("successful_inference_refreshes_readiness_without_restarting_worker") {
        return;
    }
    let _lock = crate::test_support::env_test_lock()
        .lock()
    ...
    runtime_readiness::report_subsystem_state(
        Subsystem::Embedder,
        SubsystemState::Failed {
            reason: "previous inference failure".into(),
        },
    );
...
fn oversized_request_does_not_mark_a_healthy_service_failed() {
    if !enter_isolated_case("oversized_request_does_not_mark_a_healthy_service_failed") {
        return;
    }
    runtime_readiness::report_subsystem_state(Subsystem::Embedder, SubsystemState::Ready);
    let mut worker = None;
...
fn expired_request_in_queue_aborts_without_inference() {
    if !enter_isolated_case("expired_request_in_queue_aborts_without_inference") {
        return;
    }
    runtime_readiness::report_subsystem_state(Subsystem::Embedder, SubsystemState::Ready);
    let mut worker = None;
    ...
    assert!(
        worker.is_none(),
        "no worker connection should be opened for expired request"
    );
    assert_eq!(embedder_state(), SubsystemState::Ready);
}
...
fn cpu_fallback_supervisor_isolates_worker_and_leaves_readiness_untouched() {
    ...
    if !enter_isolated_case(
        "cpu_fallback_supervisor_isolates_worker_and_leaves_readiness_untouched",
    ) {
        return;
    }
    let _lock = crate::test_support::env_test_lock()
...
fn worker_idle_drop_exit_is_detected_and_respawned_transparently() {
    ...
    if !enter_isolated_case("worker_idle_drop_exit_is_detected_and_respawned_transparently") {
        return;
    }
    let _lock = crate::test_support::env_test_lock()
        .lock()
    ...
    assert_ne!(
        initial_pid, new_pid,
        "Supervisor must have spawned a fresh worker process"
    );
    if let Some(active) = worker.take() {
        active.shutdown();
    }
            ...
            Ok(WireRequest::Embed { request_id, texts }) => {
                if std::env::var("AXON_902547_FAKE_FAIL").as_deref() == Ok("true") {
                    respond(
                        ...
                        request_id,
                        None,
                        Some("synthetic inference allocation failure".into()),
                    );
                } else {
                    respond(
                        &mut stream,
                        request_id,
                        Some(vec![vec![1.0]; texts.len()]),
                        None,
                    );
                }
            }
