...
fn test_full_pipeline_loop() {
    ...
    queue
        .push(&path, 12345, "test_trace", 0, 0, false)
    ...
    let indexed_count = graph
        .query_count("SELECT count(*) FROM File WHERE status = 'indexed'")
        .expect("Query failed");
    ...
    assert!(
        symbol_count > 0,
        "At least one symbol (the module) should be extracted"
    );
    ...
    assert!(
        contains_count > 0,
        "At least one containment edge should be persisted"
    );

    let chunk_count = graph
        .query_count("SELECT count(*) FROM Chunk")
        .expect("Query failed");
    assert!(
        chunk_count > 0,
        "At least one chunk should be derived from indexed symbols"
    );
