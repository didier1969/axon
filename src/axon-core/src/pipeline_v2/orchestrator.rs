//! Pipeline A orchestrator (CPT-AXO-054).
//!
//! Wires A1 → A2 → A3 stages through bounded channels and per-stage worker
//! pools. Exposed as [`spawn_pipeline_a`], which returns a [`PipelineAHandles`]
//! the caller uses to push paths in and pull enrollment receipts out.
//!
//! This slice (S5 of REQ-AXO-289) lands the end-to-end wiring with A3
//! restricted to its S3c scope (IndexedFile UPSERT only). Symbol / AGE /
//! Chunk persistence lands in S3d via the same channel topology — the
//! orchestrator does not need to change to accommodate that.
...
pub fn spawn_pipeline_a(
    ...
    spawn_stage_workers(
        ...
        move |parsed| {
            ...
            async move { a3_enroll(parsed, store).await }
        },
        metrics_a3.clone(),
    );
...
mod tests {
    ...
    #[tokio::test]
    async fn pipeline_a_end_to_end_persists_indexed_file_for_a_rust_fixture() {
        ...
        let mut handles = spawn_pipeline_a(counts, caps, store.clone());
        ...
        // IndexedFile row must be in PG/DuckDB after A3 enrolled the file.
        let in_db = store
            .query_count(&format!(
    ...
    async fn pipeline_a_records_error_metrics_on_unparseable_extension() {
        ...
        let store = Arc::new(crate::tests::test_helpers::create_test_db().unwrap());
        let handles = spawn_pipeline_a(counts, PipelineChannelCaps::default(), store);
