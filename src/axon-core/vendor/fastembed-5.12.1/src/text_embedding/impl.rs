...
fn resolved_intra_threads() -> Result<usize> {
    if let Some(threads) = std::env::var("AXON_ORT_INTRA_THREADS")
        .ok()
...
impl TextEmbedding {
    fn run_encoding_batch(&mut self, encodings: &[Encoding]) -> Result<SingleBatchOutput> {
        ...
        encodings.iter().for_each(|encoding| {
            ...
            let raw_length = mask
                ...
                .sum::<usize>()
                .clamp(1, ids.len());
            let pad_id = ids.get(raw_length).copied().unwrap_or(0);
            let pad_type_id = type_ids.get(raw_length).copied().unwrap_or(0);

            ids_array.extend(ids.iter().take(raw_length).map(|x| *x as i64));
            ids_array.extend(std::iter::repeat_n(
                pad_id as i64,
                encoding_length.saturating_sub(raw_length),
            ));

            mask_array.extend(mask.iter().take(raw_length).map(|x| *x as i64));
            mask_array.extend(std::iter::repeat_n(
                0_i64,
                encoding_length.saturating_sub(raw_length),
            ));

            type_ids_array.extend(type_ids.iter().take(raw_length).map(|x| *x as i64));
            type_ids_array.extend(std::iter::repeat_n(
                pad_type_id as i64,
                encoding_length.saturating_sub(raw_length),
            ));
        });
    ...
    pub fn transform_encoded(&mut self, encodings: &[Encoding]) -> Result<EmbeddingOutput> {
        if encodings.is_empty() {
            return Ok(EmbeddingOutput::new(Vec::new()));
        }

        Ok(EmbeddingOutput::new(vec![
            self.run_encoding_batch(encodings)?
        ]))
    }
    ...
    pub fn try_new(options: TextInitOptions) -> Result<Self> {
        ...
        let threads = resolved_intra_threads()?;
...
mod tests {
    ...
    #[test]
    fn resolves_intra_threads_from_axon_env_override() {
        unsafe {
            std::env::set_var("AXON_ORT_INTRA_THREADS", "3");
        }

        let threads = resolved_intra_threads().expect("resolve intra threads");
        assert_eq!(threads, 3);
